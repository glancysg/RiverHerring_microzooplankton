...
Date_bin <- Date_bin_relational%>% # change the date into a format that we can do math on 
  ...
  rename( IFCB_ID = "bin") # renames bin to match the summary file that has the counts in it. 
...
Fish_1 <- Fish_Num_date %>% #change column names to match other sheet and date to a format that we can do math with
  ...
  unite("Lake_ID", Location, year_day, remove = FALSE) #use new date to create a time space identifier


####
#remove extra columns
Fish_1 <- Fish_1 %>%
  select(-File_Name, -Measurement)
...
#remove extra columns
Fish_2 <- Fish_2 %>%
  select(-ID, -Time, -Cohort)

#create a temporary df to indicate the maximum age which is the age the day they were collected. 
...
Fish_2 <- drop_na(Fish_2)
  ...
  Fish_Trial <- full_join(Fish_2, distinct(Fish_1), by = "Fish_Num")
...
# the above results in 34 observations. 
...
Fish<- Fish%>% #this requires data from both fish files, do after join.
  mutate ( back_year_day = year_day - Collect_age + age_day) %>%
